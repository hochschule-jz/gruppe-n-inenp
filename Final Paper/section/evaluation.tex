...
\textbf{Verbindungsabbrüche.} Die Anbindung des ESP32 über \ac{wlan} war zeitweise instabil, insbesondere bei Nutzung eines mobilen Hotspots. Abbrüche der \ac{wlan}- oder \ac{mqtt}-Verbindung führten dazu, dass einzelne Statusänderungen nicht übertragen wurden. Als Lösung wurde in der Firmware eine automatische Wiederverbindung implementiert, die sowohl die \ac{wlan}- als auch die \ac{mqtt}-Verbindung bei einem Abbruch erneut aufbaut, bevor weitere Nachrichten gesendet werden.

\textbf{Schwankende Sensorwerte (\enquote{Flackern}).} Die reflexionsbasierte Messung des TCRT5000 hängt von der Beschaffenheit der Fahrzeugunterseite ab. Stark reflektierende oder sehr dunkle Unterböden führten zu schwankenden Rohwerten und damit zu kurzzeitig wechselnden Belegungszuständen. Da ausschließlich tatsächliche Zustandswechsel publiziert werden, wurde dieses Flackern durch eine geeignete Schwellenwertkalibrierung am Sensor reduziert.

\textbf{Kegelförmige Messempfindlichkeit.} Der TCRT5000 besitzt einen eng begrenzten, kegelförmigen Erfassungsbereich mit geringer Reichweite. Fahrzeuge, die nicht mittig über dem Sensor positioniert waren, wurden teilweise nicht zuverlässig erkannt. Eine sorgfältige Platzierung des Sensors unter der Stellplatzmitte verbesserte die Erkennungsrate.

\textbf{Lichtverhältnisse.} Wechselnde Umgebungslichtverhältnisse, insbesondere Sonnenlicht mit hohem Infrarotanteil, können reflexionsbasierte \ac{ir}-Sensoren beeinflussen. Im Modellbetrieb war dieser Effekt gering; für einen realen Außeneinsatz wäre jedoch eine robustere Sensorik (etwa \ac{tof}-Sensoren) zu prüfen.
...
Über alle drei Garagengrößen liegen die einmaligen Investitionskosten zwischen rund 7.600\,EUR (200 Stellplätze) und 13.800\,EUR (500 Stellplätze); die laufenden Betriebskosten betragen lediglich rund 470 bis 890\,EUR pro Jahr. Der geringe \ac{opex}-Anteil bestätigt eine zentrale Annahme der Arbeit: Der durchgehend ereignisgetriebene, serverless Aufbau verursacht im Verhältnis zu den potenziellen Parkerlösen nur sehr geringe laufende Cloud-Kosten. Die Kostenstruktur (Abbildung~\ref{fig:cost_breakdown}) wird damit klar von den einmaligen \ac{capex} dominiert.

Das wichtigste Ergebnis ist der Break-Even-Punkt: In allen Szenarien trägt sich das System bereits ab einer Auslastungssteigerung von nur rund 0{,}03 bis 0{,}04 Prozentpunkten (Abbildung~\ref{fig:breakeven}). Dieser Schwellenwert liegt weit unterhalb selbst des konservativsten betrachteten Szenarios (+1\,pp) und deutlich unter den in der Literatur berichteten Effekten von Smart-Parking-Systemen \citep{wangEconomicsSmartParking2025}. Daraus folgt, dass sich die Investition unter praktisch allen plausiblen Annahmen amortisiert; entsprechend sind \ac{roi} und Amortisationsdauer über alle Szenarien hinweg deutlich positiv bzw.\ liegen weit innerhalb des ersten Betriebsjahres.
